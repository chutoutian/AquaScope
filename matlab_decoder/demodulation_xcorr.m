...
for fileIdx = 1:numFiles
    ...
    for m = 0
        ...
        uc_start = chirp(true, SF, BW, FS, 0, 0, 0);
        uc_mid = chirp(true,SF,BW,FS,gt_transmitted_bytes(numsym/2 +m ),0,0);
        downversion_symbol_first_three = downversion_symbol(1:3*Ns);

        downversion_symbol_mid = downversion_symbol((numsym / 2 + 3 + m) * Ns+1 : (numsym / 2 + 4 +m) * Ns);

        xcorr_index = synchronization_xcorr(downversion_symbol_first_three,uc_start,Ns,true);
        xcorr_index_mid = synchronization_xcorr(downversion_symbol_mid,uc_mid,Ns,false);
        time_offset_xcorr = [time_offset_xcorr; xcorr_index];
        %%%%%%%% downsample  
        ...
        for i = 5:(numsym +4)/2 -1 + m
            ...
            %index = mod((max_index+M*10-1)/10, M);
        ...
        diff_bin_index = mod(bin_index_ori - gt_transmitted_bytes,M);
        diff_bin_index = min(diff_bin_index, M-diff_bin_index);
        %figure;
        %plot(diff_bin_index);
        %filename = sprintf('figures/diff_to_gt_xcorr_%d_%d.jpg',fileIdx,SF);
        %saveas(gcf, filename, 'jpg');
        ...
        symbols_g = gray_coding(bin_index_ori,SF);
